...
class SchoolTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        //store all the schools
        schools = SchoolDataController.shared.schoolData
        let timer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true, block: { timer in
            DispatchQueue.main.async {
                self.schools = SchoolDataController.shared.schoolData
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let cell = tableView.dequeueReusableCell(withIdentifier: cellIdentifier, for: indexPath) as? SchoolTableViewCell else{
            fatalError("The dequeued cell is not an instance of SchoolTable.")
            

        }
        let school = schools[indexPath.row]

        cell.lblSchoolName.text = school.school_name
